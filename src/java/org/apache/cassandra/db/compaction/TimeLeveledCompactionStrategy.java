...
public class TimeLeveledCompactionStrategy extends AbstractCompactionStrategy
{
    private static final Logger logger = LoggerFactory.getLogger(TimeLeveledCompactionStrategy.class);
    ...
    public static final int DEFAULT_LEVEL_FANOUT_SIZE = 5;


    // add for time window
    protected static final TimeUnit DEFAULT_COMPACTION_WINDOW_UNIT = TimeUnit.HOURS;
    ...
    // the same as compaction_window_size in timeWindowCompactionStrategy
    ...
    @VisibleForTesting
    final TimeLeveledManifest manifest;
    private final int maxSSTableSizeInMB;
    private final int levelFanoutSize;

    public TimeLeveledCompactionStrategy(ColumnFamilyStore cfs, Map<String, String> options)
    {
        super(cfs, options);
        int configuredMaxSSTableSize = 160;
        int configuredLevelFanoutSize = DEFAULT_LEVEL_FANOUT_SIZE;
        int configuredSSTableWindowSize = DEFAULT_COMPACTION_WINDOW_SIZE;
        TimeUnit configuredSSTableWindowUnit = DEFAULT_COMPACTION_WINDOW_UNIT;
        TimeUnit configuredTimestampResolution = DEFAULT_TIMESTAMP_RESOLUTION;
        ...
        if (options != null)
        {
            ...
            if (options.containsKey(COMPACTION_WINDOW_SIZE_KEY))
            {
                configuredSSTableWindowSize = Integer.parseInt(options.get(COMPACTION_WINDOW_SIZE_KEY));
            }

            if (options.containsKey(COMPACTION_WINDOW_UNIT_KEY))
            {
                configuredSSTableWindowUnit = TimeUnit.valueOf(options.get(COMPACTION_WINDOW_UNIT_KEY));
            }

            if (options.containsKey(TIMESTAMP_RESOLUTION_KEY))
            {
                configuredTimestampResolution = TimeUnit.valueOf(options.get(TIMESTAMP_RESOLUTION_KEY));
            }

            if (configuredTimestampResolution != DEFAULT_TIMESTAMP_RESOLUTION)
                logger.warn("Using a non-default timestamp_resolution {} - are you really doing inserts with USING TIMESTAMP <non_microsecond_timestamp> (or driver equivalent)?", configuredTimestampResolution.toString());
        }

        maxSSTableSizeInMB = configuredMaxSSTableSize;
        levelFanoutSize = configuredLevelFanoutSize;
        sstableWindowSize = configuredSSTableWindowSize;
        sstableWindowUnit = configuredSSTableWindowUnit;
        timestampResolution = configuredTimestampResolution;

        long firstTimeWindow = timestampResolution.convert(sstableWindowSize, sstableWindowUnit);

        manifest = new TimeLeveledManifest(cfs, this.maxSSTableSizeInMB, this.levelFanoutSize, firstTimeWindow, localOptions);
    ...
    public AbstractCompactionTask getNextBackgroundTask(int gcBefore)
    {
        ...
        {
            OperationType op;
            TimeLeveledManifest.CompactionCandidate candidate = manifest.getCompactionCandidates();
            if (candidate == null)
            {
                ...
                candidate = new TimeLeveledManifest.CompactionCandidate(Collections.singleton(sstable),
            ...
            if (txn != null)
            {
                TimeLeveledCompactionTask newTask = new TimeLeveledCompactionTask(cfs, txn, candidate.level, gcBefore, candidate.maxSSTableBytes, false);
    ...
    public synchronized Collection<AbstractCompactionTask> getMaximalTask(int gcBefore, boolean splitOutput)
    {
        ...
        if (txn == null)
            return null;
        return Arrays.<AbstractCompactionTask>asList(new TimeLeveledCompactionTask(cfs, txn, 0, gcBefore, getMaxSSTableBytes(), true));
    ...
    public AbstractCompactionTask getUserDefinedTask(Collection<SSTableReader> sstables, int gcBefore)
    {
        ...
        return new TimeLeveledCompactionTask(cfs, transaction, level, gcBefore, level == 0 ? Long.MAX_VALUE : getMaxSSTableBytes(), false);
    }
    ...
    public AbstractCompactionTask getCompactionTask(LifecycleTransaction txn, int gcBefore, long maxSSTableBytes)
    {
        ...
        return new TimeLeveledCompactionTask(cfs, txn, level, gcBefore, maxSSTableBytes, false);
    }
    ...
    protected final static ImmutableList<TimeUnit> validWindowTimeUnits = ImmutableList.of(TimeUnit.SECONDS, TimeUnit.MINUTES, TimeUnit.HOURS, TimeUnit.DAYS);

    public static Map<String, String> validateOptions(Map<String, String> options) throws ConfigurationException
    {
        Map<String, String> uncheckedOptions = AbstractCompactionStrategy.validateOptions(options);

        String optionValue = options.get(COMPACTION_WINDOW_UNIT_KEY);
        try
        {
            if (optionValue != null)
