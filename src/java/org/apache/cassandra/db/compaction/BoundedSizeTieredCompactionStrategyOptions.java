...
public class BoundedSizeTieredCompactionStrategyOptions
{
    protected static final long DEFAULT_MIN_SSTABLE_SIZE = 50L * 1024L * 1024L;
    protected static final double DEFAULT_BUCKET_LOW = 0.5;
    protected static final double DEFAULT_BUCKET_HIGH = 1.5;
    protected static final long DEFAULT_MAX_SSTABLE_SIZE_TO_COMPACTION = 50L * 1024L * 1024L * 1024L; // 50G
    ...
    protected long maxSSTableSize; // if sstable is larger, don't compact

    public BoundedSizeTieredCompactionStrategyOptions(Map<String, String> options)
    {
        ...
        maxSSTableSize = optionValue == null ? DEFAULT_MAX_SSTABLE_SIZE_TO_COMPACTION : Long.parseLong(optionValue);
        if (maxSSTableSize < 0)
            maxSSTableSize = Long.MAX_VALUE;
    }

    public BoundedSizeTieredCompactionStrategyOptions()
    {
    ...
    {
        String min_size = options.get(MIN_SSTABLE_SIZE_KEY);
        String max_size = options.get(MAX_SSTABLE_SIZE_KEY);
        try
        {
            long minSSTableSize = min_size == null ? DEFAULT_MIN_SSTABLE_SIZE : Long.parseLong(min_size);
            long maxSSTableSize = max_size == null ? DEFAULT_MAX_SSTABLE_SIZE_TO_COMPACTION : Long.parseLong(max_size);
            if (minSSTableSize < 0)
            {
                throw new ConfigurationException(String.format("%s must be non negative: %d", MIN_SSTABLE_SIZE_KEY, minSSTableSize));
            }
            if (maxSSTableSize >= 0 && maxSSTableSize < minSSTableSize){
                throw new ConfigurationException(String.format("%s must be non negative: %d", MAX_SSTABLE_SIZE_KEY, maxSSTableSize));
            }
        }
        catch (NumberFormatException e)
        {
            throw new ConfigurationException(String.format("%s or %s is not a parsable int (base10) for %s and %s", min_size, max_size, MIN_SSTABLE_SIZE_KEY, MAX_SSTABLE_SIZE_KEY), e);
        }
        ...
        uncheckedOptions.remove(MAX_SSTABLE_SIZE_KEY);
