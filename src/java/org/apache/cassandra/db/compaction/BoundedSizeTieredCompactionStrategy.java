...
public class BoundedSizeTieredCompactionStrategy extends AbstractCompactionStrategy
{
    private static final Logger logger = LoggerFactory.getLogger(BoundedSizeTieredCompactionStrategy.class);
    ...
    protected BoundedSizeTieredCompactionStrategyOptions sizeTieredOptions;
    protected volatile int estimatedRemainingTasks;
    @VisibleForTesting
    protected final Set<SSTableReader> sstables = new HashSet<>();

    public BoundedSizeTieredCompactionStrategy(ColumnFamilyStore cfs, Map<String, String> options)
    {
        super(cfs, options);
        this.estimatedRemainingTasks = 0;
        this.sizeTieredOptions = new BoundedSizeTieredCompactionStrategyOptions(options);
    }
    ...
    private synchronized List<SSTableReader> getNextBackgroundSSTables(final int gcBefore)
    {
        ...
        List<List<SSTableReader>> buckets = getBuckets(createSSTableAndLengthPairs(candidates),
                                                       ...
                                                       sizeTieredOptions.maxSSTableSize);
        ...
        List<SSTableReader> mostInteresting = mostInterestingBucket(buckets, minThreshold, maxThreshold, sizeTieredOptions.maxSSTableSize);
     ...
     */
    public static List<SSTableReader> mostInterestingBucket(List<List<SSTableReader>> buckets, int minThreshold, int maxThreshold, long maxSSTableSize)
    {
        ...
        {
            Pair<List<SSTableReader>, Double> bucketAndHotness = trimToThresholdWithHotness(bucket, maxThreshold, maxSSTableSize);
     ...
     **/
    @VisibleForTesting
    static Pair<List<SSTableReader>, Double> trimToThresholdWithHotness(List<SSTableReader> bucket, int maxThreshold, long maxSSTableSize)
    {
        ...
        for (int i = 0; i < index; i++) {
            sumSize += bucket.get(i).onDiskLength();
            if (sumSize > maxSSTableSize) {
                index = i;
                break;
            }
        }
        ...
        List<SSTableReader> prunedBucket = bucket.subList(0, Math.min(bucket.size(), Math.min(index, maxThreshold)));
     ...
     * If the total size of a bucket exceeds maxSSTableSize, don't add new pair into it.
     */
    public static <T> List<List<T>> getBuckets(Collection<Pair<T, Long>> files, double bucketHigh, double bucketLow, long minSSTableSize, long maxSSTableaSize)
    {
        ...
        for (Pair<T, Long> pair: sortedFiles)
        {
            ...
            if (size < maxSSTableaSize)
            {
                ...
                for (Map.Entry<Long, List<T>> entry : buckets.entrySet())
                {
                    ...
                    {
                        long totalSize = bucket.size() * oldAverageSize + size;
                        // remove and re-add under new new average size
                        buckets.remove(oldAverageSize);
                        long newAverageSize = totalSize / (bucket.size() + 1);
    ...
    public static Map<String, String> validateOptions(Map<String, String> options) throws ConfigurationException
    {
        ...
        uncheckedOptions = BoundedSizeTieredCompactionStrategyOptions.validateOptions(options, uncheckedOptions);
    ...
    {
        return String.format("BoundedSizeTieredCompactionStrategy[%s/%s]",
